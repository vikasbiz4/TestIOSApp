...
- (void)viewDidLoad {
    ...
    NSLog(@"width=%f",scrollView.bounds.size.height);
    
    
    
    NSString *boxerFileName = [[NSBundle mainBundle] pathForResource:@"config" ofType:@"properties"];
...
//    NSLog(@"deviceDataArray=%@",deviceDataArray);
     NSArray *keys = [[NSArray alloc] init];
     NSArray *values = [[NSArray alloc] init];
    NSDictionary *dictonary=[[NSDictionary alloc]init];
    for (int i=0; i<deviceDataArray.count; i++) {
//        NSLog(@"data=%@",[deviceDataArray[i] componentsSeparatedByString:@"="].lastObject);
        [dictonary setValue:[deviceDataArray[i] componentsSeparatedByString:@"="].lastObject forKey:[deviceDataArray[i] componentsSeparatedByString:@"="].firstObject];
    }
    NSLog(@"data=%@",dictonary);
    
    
    CGRect fullScreenRect=[[UIScreen mainScreen] applicationFrame];
    NSLog(@"screen height=%f",fullScreenRect.size.height);
    [scrollView setBounds:CGRectMake(0, 0, 320, 500)];
    if (fullScreenRect.size.height<scrollView.bounds.size.height) {
        [scrollView setContentSize:CGSizeMake(300, scrollView.bounds.size.height+150)];
    }
